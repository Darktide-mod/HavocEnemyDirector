...
local n=win.HED_TEMPLATE_IO_MultiByteToWideChar(65001,8,path,#path,nil,0)
local wide=ffi.new("unsigned short[?]",n+1); win.HED_TEMPLATE_IO_MultiByteToWideChar(65001,8,path,#path,wide,n)
local handle=win.HED_TEMPLATE_IO_CreateFileW(wide,2147483648,1,nil,3,128,nil)
assert(handle~=ffi.cast("void*",-1))
saved=P.save_document(doc,doc.name,true); win.HED_TEMPLATE_IO_CloseHandle(handle)
